...
import artifactToken from 'arcoiris/artifacts/ERC721PresetMinterPauserAutoId.sol/ERC721PresetMinterPauserAutoId.json';
import artifactArcoiris from 'arcoiris/artifacts/Arcoiris.sol/Arcoiris.json';
import artifactQuizMC from 'arcoiris/artifacts/QuizMC.sol/QuizMC.json';
...
function Page() {
  ...
  async function verifyGuessCorrect() {
    ...
    const isVerified = await verifyGuess(state.quizMC.target, answers);
    ...
    <>
      <main className={styles.main}>
        ...
        {state.correct.hashes && (
          <div>
            <div>first: {state.correct.hashes[0]}</div>
            <div>second: {state.correct.hashes[1]}</div>
          </div>
        )}
        ...
        {state.correct.hasRevealed && (
          <div>
            <div>
              <span>first answer hash: {state.correct.guess[0].substring(0, 5)}...</span>
              <Copy text={state.correct.guess[0]}/>
            </div>
            <div>
              <span>second answer hash: {state.correct.guess[1].substring(0, 5)}...</span>
          ...
          <div className={styles.cards}>
            <div className={styles.options}>
              ...
              {state.ceremonyID !== undefined && state.alice.contribution === 0 && (
                <button onClick={contributeAlice}>contribute</button>
              )}
            ...
            <div className={styles.options}>
              ...
              {state.ceremonyID !== undefined && state.bob.contribution === 0 && (
                <button onClick={contributeBob}>contribute</button>
              )}
            ...
            <div className={styles.options}>
              ...
              {state.ceremonyID !== undefined && state.tony.contribution === 0 && (
                <button onClick={contributeTony}>contribute</button>
              )}
